...
class get_features:
    ...
    def featurize_ecg(self, recording, sample_freq):
        ...
        feature_list = []
        feature_name = []
        intervals = {}  # Store the calculated intervals here
        try:
        ...
        # Calculate RR, PP, QQ, SS, and TT intervals
        if self.rpeak_int:
            if analysis:
                rr_intervals = np.diff(r_peaks) / sample_freq
                intervals["rr_intervals"] = rr_intervals

        if self.ppeak_int:
            if analysis:
                pp_intervals = np.diff(p_peaks) / sample_freq
                intervals["pp_intervals"] = pp_intervals

        if self.qpeak_int:
            if analysis:
                qq_intervals = np.diff(q_peaks) / sample_freq
                intervals["qq_intervals"] = qq_intervals

        if self.speak_int:
            if analysis:
                ss_intervals = np.diff(s_peaks) / sample_freq
                intervals["ss_intervals"] = ss_intervals

        if self.tpeak_int:
            if analysis:
                tt_intervals = np.diff(t_peaks) / sample_freq
                intervals["tt_intervals"] = tt_intervals

            
        feature_list = np.asarray(feature_list)
        feature_name = np.asarray(feature_name)
         
        return feature_list,feature_name, intervals, [p_peaks,q_peaks,r_peaks,s_peaks,t_peaks]
