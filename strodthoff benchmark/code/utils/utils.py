...
def load_raw_data_ptbxl(df, sampling_rate, path):
    if sampling_rate == 100:
        # # sex0
        # if os.path.exists(path + 'raw100_sex0.npy'):
        #     data = np.load(path+'raw100_sex0.npy', allow_pickle=True)
        # sex1
        # if os.path.exists(path + 'raw100_sex1.npy'):
        #     data = np.load(path+'raw100_sex1.npy', allow_pickle=True)
        #Segmented
        # if os.path.exists(path + 'raw100_segmented.npy'):
        #     data = np.load(path+'raw100_segmented.npy', allow_pickle=True)
        # #Orig code    
        if os.path.exists(path + 'raw100.npy'):
            data = np.load(path+'raw100.npy', allow_pickle=True)
        else:
...
# def compute_label_aggregations(df, folder, ctype, sex_filter=None):
def compute_label_aggregations(df, folder, ctype):
    # Optionally filter data based on sex
...
def generate_ptbxl_summary_table(selection=None, folder='../output/'):
    ...
    # exps = ['test', 'test_sex0', 'test_sex1', 'test_segmented']
    exps = ['test_segmented']
    metric1 = 'macro_auc'
    print("selection is")
    print(selection)
