...
class LimeTimeSeriesExplanation(object):
    ...
    def explain_instance(self,
        ...
        Args:
            ...
            predicted_probabilities: Matrix of predicted probabilities for each perturbed instance.
                Shape should be (num_samples, num_classes), where num_samples is the number of
                perturbed instances and num_classes is the number of classes.
        ...
        data, _, distances = self.__data_labels_distances(timeseries, predicted_probabilities, num_samples, num_slices,
                                                        replacement_method)
        ret_exp = explanation.Explanation(domain_mapper=domain_mapper, class_names=self.class_names)
        ret_exp.predict_proba = predicted_probabilities[0]
        for label in labels:
            (ret_exp.intercept[int(label)],
            ret_exp.local_exp[int(label)],
            ret_exp.score, ret_exp.local_pred) = self.base.explain_instance_with_data(data, predicted_probabilities, distances, label,
                                                                                    num_features,
    ...
    def __data_labels_distances(cls,
        ...
        values_per_slice = int(math.ceil(len(time_series) / num_slices))
        ...
        for i, size in enumerate(sample, start=1):
            ...
            for i, inact in enumerate(inactive, start=1):
                ...
                if replacement_method == 'noise':
                    ...
                    tmp_series[index:(index + values_per_slice)] = np.random.uniform(0, 0, len(
        ...
        labels = ', '.join([classes[i] for i, prob in enumerate(predicted_probabilities[0]) if prob >= 0.5])
